...
void preparar_hamburguesa(){
    ...
    mensaje orden;
    while(1){
		msgrcv(qid,&orden,sizeof(mensaje)-sizeof(double),HAMBURGUESA,0);
		printf("Preparando hamburguesa..\n");
		sleep(TIEMPO_HAMBURGUESA);
		orden.tipo = orden.nro_pedido;
		printf("Hamburguesa lista.\n");
		msgsnd(qid,&orden,sizeof(mensaje)-sizeof(double),0);
    }
}
...
void preparar_menu_vegano(){
    ...
    mensaje orden;
    while(1){
		msgrcv(qid,&orden,sizeof(mensaje)-sizeof(double),VEGANO,0);
...
void preparar_papas(){
    ...
    mensaje orden;
    while(1){
		msgrcv(qid,&orden,sizeof(mensaje)-sizeof(double),PAPAS,0);
...
void recibir(){
    ...
    while(1){
		...
		//leo un pedido
		msgrcv(qid,&pedido,sizeof(mensaje)-sizeof(double),PEDIDO,0);
		
		//si es un pedido vip atiendo a todos los vips
		while(pedido.VIP==1){
			pedido.tipo = pedido.combo;
			msgsnd(qid,&pedido,sizeof(mensaje)-sizeof(double),0);
			msgrcv(qid,&pedido,sizeof(mensaje)-sizeof(double),PEDIDO,0);
		}
